    ...
    def fit(self, show=False, save=False):
        ...
        for j in range(self.PSFnumber):
            ...
            for t in range(len(self.trajectory)):
                ...
                m2 = int(np.minimum(self.canvas[1] - 1, np.maximum(1, np.floor(self.trajectory[t].real))))
                M2 = int(m2 + 1)
                m1 = int(np.minimum(self.canvas[0] - 1, np.maximum(1, np.floor(self.trajectory[t].imag))))
                M1 = int(m1 + 1)
